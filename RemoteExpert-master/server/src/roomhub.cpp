...
RoomHub::RoomHub(QObject* parent) : QObject(parent),dbManager_(DatabaseManager::instance()),dataForwarder_(this,this){}
RoomHub::~RoomHub(){}

//Part 1.Tcp Server Manage
// 实现监听功能
ClientCtx* RoomHub::getClient(QTcpSocket *sock) const
{
    return clients_.value(sock);
}
QList<QTcpSocket*>RoomHub::getClientsInRoom(const QString &roomId, QTcpSocket *except) const
{
...
int RoomHub::getRoomCount() const
{
    return rooms_.uniqueKeys().size();
}
int RoomHub::getClientCount() const
{
...
void RoomHub::onReadyRead()
{
    ...
    QByteArray newData = sock->readAll();  // 读取新收到的数据
        ...
        dataForwarder_.processData(sock,buf);
}
...
void RoomHub::handlePacket(ClientCtx* c, const Packet& p)
{
    if(p.type == MSG_REGISTER)
    {
        ...
        bool registerSuccess = dbManager_.addUser(username,password,email,phone,userType);
        if(registerSuccess)
        {
            QJsonObject resp
            {
    ...
    if(//username == "factory" && password =="123456"
            dbManager_.validateUser(username,password,userType))
    {
        c->user =username;
...
//    }
    ...
    if (p.type == MSG_TEXT || p.type == MSG_DEVICE_DATA ||
        ...
        p.type == MSG_CONTROL || p.type == MSG_QOS_NACK)
    {
        dataForwarder_.processPacket(c,p);
        return;
    }
